"""1D-CNN DDoS classifier (PyTorch parity with `models/cnn_tf.py`)."""
...
def train(epochs: int, seed: int) -> None:
    ...
    model = CNN1D(X_tr.shape[1], n_cls).to(dev)
    ...
    model.eval()
    with torch.no_grad():
        logits = model(torch.from_numpy(X_te).unsqueeze(1).to(dev)).cpu().numpy()
    preds = logits.argmax(1)
    rep = classification_report(y_te, preds, target_names=le.classes_, digits=3, zero_division=0)
    print(rep)
    (REPORTS / "cnn_pt_report.txt").write_text(rep)
    torch.save(model.state_dict(), ART / "cnn_pt.pt")
