"""1D-CNN DDoS classifier (TensorFlow/Keras)."""
...
def build_model(in_dim: int, n_cls: int) -> tf.keras.Model:
    m = models.Sequential([
...
def train(epochs: int) -> None:
    ...
    model.fit(
        X_tr[..., None], y_tr,
        validation_data=(X_te[..., None], y_te),
        epochs=epochs, batch_size=512, verbose=2,
        callbacks=[callbacks.EarlyStopping(patience=3, restore_best_weights=True)],
    )
    preds = model.predict(X_te[..., None], verbose=0).argmax(1)
    rep = classification_report(y_te, preds, target_names=le.classes_, digits=3, zero_division=0)
    print(rep)
    (REPORTS / "cnn_tf_report.txt").write_text(rep)
    model.save(ART / "cnn_tf.keras")
